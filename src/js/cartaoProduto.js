...
export function renderizarCatalogo() {
    ...
    <img src="src/assets/img/${produtoCatalogo.imagem}" alt="produto-1" class="group-hover:scale-110 duration-300 my-3 rounded-lg">
    <p class="text-sm">${produtoCatalogo.marca}</p>
    <p class="text-sm">${produtoCatalogo.nome}</p>
    <p class="text-sm">$${produtoCatalogo.preco}</p>
    <button id="adicionar-${produtoCatalogo.id}" class="bg-slate-950 hover:bg-slate-700 text-slate-200 rounded-lg"><i class="fa-solid fa-cart-plus"></i></button>
    </div>`;
